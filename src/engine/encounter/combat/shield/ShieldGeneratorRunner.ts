...
export default class ShieldGeneratorRunner {
    ...
    public step(deltaMs: number): void {
        if (!Number.isFinite(deltaMs) || deltaMs < 0) {
            throw new Error("Shield emitter deltaMs must be non-negative: " + String(deltaMs));
        }

        const emitter = this.state.combat.shieldGenerator;

        if (emitter) {
            advanceShieldGenerator(emitter, deltaMs);
        }

        const expiredShield = advanceActiveShield(this.state, deltaMs);
        ...
        this.emit({
            ...
            shield: expiredShield,

            outcome: PLAYER_SHIELD_END_OUTCOME.EXPIRED,
        });
    }
}

export function advanceShieldGenerator(emitter: ShieldGeneratorState, deltaMs: number): void {
    if (emitter.status === SHIELD_GENERATOR_STATUS.BROKEN) {
        return;
    }
    ...
    switch (emitter.phase) {
        ...
        case SHIELD_GENERATOR_PHASE.COOLDOWN: {
            ...
            const elapsedMs = emitter.phaseElapsedMs + deltaMs;

            if (elapsedMs >= definition.cooldownDurationMs) {
            ...
            emitter.phaseElapsedMs = elapsedMs;

            return;
        }

        default: {
            const exhaustivePhase: never = emitter.phase;
...
export function advanceActiveShield(state: EncounterState, deltaMs: number): ActiveShieldState | undefined {
    const shield = state.combat.activeShield;
