...
export type MissileSignatureAnalysisProfile =
    (typeof MISSILE_SIGNATURE_ANALYSIS_PROFILE)[keyof typeof MISSILE_SIGNATURE_ANALYSIS_PROFILE];

export type MissileSignatureAnalysisResult = {
    identification: ResolvedMissileSignatureIntel;

    confidence: MissileSignatureAnalysisConfidence;
};
...
const OUTCOMES_BY_PROFILE: Record<MissileSignatureAnalysisProfile, readonly WeightedAnalysisOutcome[]> = {
    [MISSILE_SIGNATURE_ANALYSIS_PROFILE.STANDARD]: [
        {
            upperBound: 0.45,
            confidence: MISSILE_SIGNATURE_ANALYSIS_CONFIDENCE.CERTAIN,
            correct: true,
        },
        {
            upperBound: 0.77,
            confidence: MISSILE_SIGNATURE_ANALYSIS_CONFIDENCE.STRONG,
            correct: true,
        },
        {
            upperBound: 0.85,
        ...
        {
            upperBound: 0.925,
            confidence: MISSILE_SIGNATURE_ANALYSIS_CONFIDENCE.WEAK,
            correct: true,
        },
        {
            upperBound: 1,
...
export function resolveMissileSignatureAnalysis({
    ...
    profile: MissileSignatureAnalysisProfile;
    ...
    if (!Number.isFinite(randomValue) || randomValue < 0 || randomValue >= 1) {
        throw new Error("Missile Science random source must return a value in [0, 1): " + randomValue);
    }

    const outcomes = OUTCOMES_BY_PROFILE[profile];

    const outcome =
        outcomes.find((candidate) => {
            return randomValue < candidate.upperBound;
        }) ?? outcomes[outcomes.length - 1];

    if (!outcome) {
        throw new Error("Missile Science analysis profile has no outcomes: " + profile);
    }

    const hypothesis = outcome.correct ? truth : getOppositeSignature(truth);

    if (outcome.confidence === MISSILE_SIGNATURE_ANALYSIS_CONFIDENCE.CERTAIN) {
        return {
            identification: {
    ...
    return {
        identification: {
            status: MISSILE_SIGNATURE_INTEL_STATUS.UNCERTAIN,
...
function getOppositeSignature(signature: MissileSignature): MissileSignature {
    switch (signature) {
