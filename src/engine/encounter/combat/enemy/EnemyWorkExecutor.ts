...
export default class EnemyWorkExecutor {
    ...
    constructor({ state, emit, crewTaskRunner }: EnemyWorkExecutorOptions) {
        this.state = state;
        this.emit = emit;
        this.crewTaskRunner = crewTaskRunner;
    }

    public start(actor: ShipEncounterActorState, intent: EnemyWorkIntent): void {
        switch (intent.kind) {
            case SHIP_CREW_TASK_KIND.DEPLOY_SHIELD:
                this.startShieldDeployment(actor, intent);

                return;

            case SHIP_CREW_TASK_KIND.PURGE_SPAM:
                this.startSpamPurging(actor, intent);
            ...
            case SHIP_CREW_TASK_KIND.OPERATE_WEAPON:
                this.startWeaponOperation(actor, intent);

                return;

            case SHIP_CREW_TASK_KIND.INTERCEPT_MISSILE:
                this.startDefenseTurretInterception(actor, intent);
    ...
    private startShieldDeployment(
        ...
        if (
            !observation ||
            observation.kind !== ENEMY_THREAT_KIND.BEAM_CANNON ||
            !emitter ||
            ...
            throw new Error("Cannot start enemy shield deployment: " + actor.id + "/" + intent.observationId);
        }

        // Same economic rule as player defensive work:
        // charge is committed when Engineer starts.
        spendPowerCoreCharge(powerCore);
    ...
    private startStickyMineClearing(
        ...
        if (!mine) {
            throw new Error(
                "Cannot start enemy sticky-mine " + "clearing: " + actor.id + "/" + intent.role + "/" + intent.mineId,
            );
        }
    ...
    private startSpamPurging(
        ...
        if (!channel) {
            throw new Error("Cannot start enemy spam purge: " + actor.id + "/" + intent.channelId);
        }
    ...
    private startDefenseTurretInterception(
        ...
        const projectile = this.state.combat.projectiles.find((candidate) => {
            return candidate.id === intent.projectileId;
        });

        if (
            !defenseTurret ||
            defenseTurret.id !== intent.defenseTurretId ||
        ...
        this.crewTaskRunner.start(actor, intent);
        ...
        defenseTurret.targetProjectileId = intent.projectileId;

        this.emit({
            type: ENCOUNTER_EVENT.ENEMY_DEFENSE_TURRET_LOADING_STARTED,

            sourceActorId: actor.id,
            defenseTurretId: defenseTurret.id,

            projectileId: intent.projectileId,

            loadDurationMs: definition.loadDurationMs,
        });
    }
    ...
    private startWeaponOperation(
        ...
        const weapon = actor.weapons.find((candidate) => {
            return candidate.id === intent.weaponId;
        });

        if (!weapon || weapon.phase !== SHIP_WEAPON_PHASE.READY) {
            throw new Error("Cannot start enemy weapon work: " + actor.id + "/" + intent.role + "/" + intent.weaponId);
        }
        ...
        switch (weapon.kind) {
            ...
            case SHIP_WEAPON_KIND.BEAM_CANNON:
                this.commitWeaponCooldown(weapon);
            ...
            default: {
                ...
                throw new Error("Unhandled enemy weapon kind: " + String(exhaustiveWeapon));
            }
        }
    ...
    private commitWeaponCooldown(weapon: ShipWeaponState): void {
        const definition = SHIP_WEAPONS[weapon.weaponId];

        if (definition.kind !== weapon.kind) {
            throw new Error("Enemy weapon definition mismatch: " + weapon.id + "/" + weapon.weaponId);
        }
