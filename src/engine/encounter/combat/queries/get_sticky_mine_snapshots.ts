...
import { getNextClearableStickyMine } from "./get_next_clearable_sticky_mine";
...
export function getStickyMineSnapshots(state: EncounterState): StickyMineSnapshot[] {
    ...
    const nextMine = getNextClearableStickyMine(state);
        ...
        state.combat.stickyMines
            ...
            .map((mine) => {
                return {
                    ...
                    isBeingCleared: reservedMineIds.has(mine.id),

                    isNextClearTarget: mine.id === nextMine?.id,
                };
            }),
    );
}
