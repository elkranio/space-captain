...
export default class CombatBeamCannonRunner {
    ...
    constructor({ stateStore, identities, emit, interruptRandomOfficerTask }: CombatBeamCannonRunnerOptions) {
    ...
    public advanceEnemyBeamCannon(
        ...
        advanceShipWeaponCooldown(beamCannon, definition.cooldownDurationMs, worldDeltaMs);
        ...
        switch (beamCannon.phase) {
            ...
            case SHIP_WEAPON_PHASE.TARGETING:
                throw new Error(`BeamCannon cannot enter targeting phase: ` + `${actor.id}/${beamCannon.id}`);
    ...
    private ensureAttackStarted(actor: ShipEncounterActorState, beamCannon: BeamCannonState): void {
        const existingAttack = this.state.combat.beamCannonAttacks.some((attack) => {
        ...
        const attack = this.createAttack(actor, beamCannon);
    ...
    private advanceCharging(actor: ShipEncounterActorState, beamCannon: BeamCannonState, deltaMs: number): void {
        const definition = this.getDefinition(beamCannon);
    ...
    private createAttack(actor: ShipEncounterActorState, beamCannon: BeamCannonState): BeamCannonAttackState {
        const existingAttack = this.state.combat.beamCannonAttacks.find((attack) => {
            return attack.sourceActorId === actor.id && attack.sourceWeaponId === beamCannon.id;
        });

        if (existingAttack) {
            throw new Error(
                `BeamCannon weapon already has active attack: ` + `${actor.id}/${beamCannon.id}/${existingAttack.id}`,
            );
        }

        const attack: BeamCannonAttackState = {
            id: this.identities.createBeamCannonAttackId(),

            designation: this.identities.createThreatDesignation("L"),
    ...
    private fire(actor: ShipEncounterActorState, beamCannon: BeamCannonState, definition: BeamCannonDefinition): void {
        const attackIndex = this.state.combat.beamCannonAttacks.findIndex((attack) => {
            return attack.sourceActorId === actor.id && attack.sourceWeaponId === beamCannon.id;
        });

        if (attackIndex < 0) {
            throw new Error(`Cannot fire beamCannon without active attack: ` + `${actor.id}/${beamCannon.id}`);
        }

        const attack = this.state.combat.beamCannonAttacks[attackIndex];

        if (!attack) {
            throw new Error(`BeamCannon attack disappeared before fire: ` + `${actor.id}/${beamCannon.id}`);
        }

        this.state.combat.beamCannonAttacks.splice(attackIndex, 1);
        ...
        if (absorbedByShield) {
            ...
            this.emit({
                type: ENCOUNTER_EVENT.PLAYER_SHIELD_ENDED,

                shield: absorbedByShield,

                outcome: PLAYER_SHIELD_END_OUTCOME.ABSORBED,
            });
    ...
    private getDefinition(beamCannon: BeamCannonState): BeamCannonDefinition {
        ...
        if (definition.kind !== beamCannon.kind) {
            throw new Error(
                `BeamCannon weapon kind does not match definition: ` + `${beamCannon.id}/${beamCannon.weaponId}`,
            );
        }
