...
export default class EncounterNavigationStore {
    ...
    public completeArrival(): void {
        ...
        if (
            navigation.kind !==
            PLAYER_SPACE_NAVIGATION_KIND.ARRIVING
        ) {
            throw new Error(
                `Cannot complete arrival from navigation state: ${navigation.kind}`,
            );
        }
    ...
    public startTravel(
        ...
        if (
            ...
            throw new Error(
                `Cannot start travel from navigation state: ${navigation.kind}`,
            );
        }
        ...
        if (!target) {
            throw new Error(
                `Travel target not found: ${targetAnchorId}`,
            );
        }
    ...
    public completeTravel(
        ...
        if (
            ...
            throw new Error(
                `Cannot complete travel from navigation state: ${navigation.kind}`,
            );
        }
    ...
    public createJumpPoint(
        ...
        const existingJumpPoint =
            this.state.anchors
                .find((anchor) => {
                    return (
                        anchor.kind ===
                        ENCOUNTER_ANCHOR_KIND.JUMP_POINT
                    );
                });

        if (existingJumpPoint) {
            throw new Error(
                `Encounter already contains jump point: ${existingJumpPoint.id}`,
            );
        }

        const id =
            `jump_point_${targetNodeId}`;
        ...
        if (
            ...
            throw new Error(
                `Cannot create duplicate encounter anchor: ${id}`,
            );
        }
