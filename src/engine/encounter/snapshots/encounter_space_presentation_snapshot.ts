...
type EncounterSpaceAnchorBasePresentationSnapshot = {
    id: EncounterAnchorBaseState["id"];

    localPosition: EncounterAnchorBaseState["localPosition"];

    position: EncounterAnchorBaseState["position"];

    perspectiveDepth: EncounterAnchorBaseState["perspectiveDepth"];
};

export type EncounterSpaceAnchorPresentationSnapshot = EncounterSpaceAnchorBasePresentationSnapshot &
    (
        | {
...
export type EncounterSpacePresentationSnapshot = {
    anchors: EncounterSpaceAnchorPresentationSnapshot[];

    actors: EncounterSpaceActorPresentationSnapshot[];
};

export function createEncounterSpacePresentationSnapshot(state: EncounterState): EncounterSpacePresentationSnapshot {
    return {
        anchors: state.anchors.map(createAnchorPresentationSnapshot),

        actors: state.actors.map(createActorPresentationSnapshot),
    };
}

function createAnchorPresentationSnapshot(anchor: EncounterAnchorState): EncounterSpaceAnchorPresentationSnapshot {
    const base = {
        id: anchor.id,
    ...
    switch (anchor.kind) {
        case ENCOUNTER_ANCHOR_KIND.STATION:
            return {
                ...
                station: {
                    objectSpriteId: anchor.station.objectSpriteId,
                },
            };
        ...
        case ENCOUNTER_ANCHOR_KIND.NAVIGATION_BEACON:
            return {
                ...
                beacon: {
                    objectSpriteId: anchor.beacon.objectSpriteId,
                },
            };
        ...
        case ENCOUNTER_ANCHOR_KIND.ASTEROID:
            return {
                ...
                asteroid: {
                    objectSpriteId: anchor.asteroid.objectSpriteId,
                },
            };
        ...
        case ENCOUNTER_ANCHOR_KIND.JUMP_POINT:
            return {
                ...
                jumpPoint: {
                    objectSpriteId: anchor.jumpPoint.objectSpriteId,
                },
            };
...
function createActorPresentationSnapshot(actor: EncounterActorState): EncounterSpaceActorPresentationSnapshot {
    return {
        kind: actor.kind,
...
function assertNeverAnchor(value: never): never {
    throw new Error(`Unhandled encounter anchor presentation kind: ${String(value)}`);
}
