...
import {
    DEBUG_START,
} from '../catalogs/debug_start';
import {
    SHIP_CHASSIS,
} from '../catalogs/ship_chassis';
import {
    SHIP_DRIVES,
...
import type {
    CreatedShipState,
} from '../../generation/ship/ShipFactory';
...
type DebugStartShipSide =
    'player' |
    'enemy';

export function createDebugStartPlayerShip():
    PlayerShipState {
    const config =
        DEBUG_START.player;
    ...
    return {
        ...
        shieldGenerator:
            ShieldGeneratorFactory.create({
                ...
                shieldGeneratorId:
                    config.shieldGeneratorId,
            }),

        weapons:
            createInstalledWeapons(
                [
                    config.weaponSlot1Id,
                    config.weaponSlot2Id,
                    config.weaponSlot3Id,
                    config.weaponSlot4Id,
                ],
                'player',
            ),
    };
...
export function createDebugStartEnemyShip():
    CreatedShipState {
    const config =
        DEBUG_START.enemy;
...
function createInstalledWeapons(
    weaponIds: ShipWeaponId[],
    side: DebugStartShipSide,
): ShipWeaponState[] {
    const occurrenceByKind:
        Partial<
            Record<
                ShipWeaponKind,
                number
...
function createWeaponRuntimeId(
    kind: ShipWeaponKind,
    side: DebugStartShipSide,
    occurrence: number,
): string {
...
function assertNever(
    ...
    throw new Error(
        'Unhandled debug-start weapon definition: ' +
