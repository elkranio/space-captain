...
// Special enemy-ship column.
// HULL is authoritative presentation. BRIDGE mirrors the player placeholder.
export default class BridgeEnemyShipSpecialColumnView {
    ...
    private buildBridgeCells(panel: Phaser.GameObjects.Container): void {
        ...
        for (let index = 0; index < 4; index += 1) {
            ...
            const cell = this.scene.add
                ...
                .setStrokeStyle(
                    1,
                    CAPTAIN_DASHBOARD_STYLE.specialColumn.cellBorderColor,
                );
    ...
    private handleDashboardUpdated(
        ...
        this.reconcileHullSegments(payload.hull.max);
        this.updateHullSegments(payload.hull.current);
    }

    private reconcileHullSegments(max: number): void {
        if (!Number.isInteger(max) || max < 0) {
            throw new Error("Enemy HULL max must be a non-negative integer: " + max);
        }
        ...
        if (segmentHeight <= 0) {
            throw new Error("Enemy HULL HP segments are too short to render");
        }
        ...
        const segmentWidth = Math.floor(
            (
                availableWidth -
                COLUMN.hullSegmentColumnGap * (columnCount - 1)
            ) / columnCount,
        );

        if (segmentWidth <= 0) {
            throw new Error("Enemy HULL HP segments are too narrow to render");
        }
