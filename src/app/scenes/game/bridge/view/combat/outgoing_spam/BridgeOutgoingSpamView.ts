...
export default class BridgeOutgoingSpamView {
    ...
    private readonly channels = new Map<string, OutgoingSpamChannelEntry>();
    ...
    private startChannel(payload: BridgeOutgoingSpamChannelStartedPayload): void {
        if (this.channels.has(payload.channelId)) {
            throw new Error("Outgoing spam channel " + "already exists: " + payload.channelId);
        }
    ...
    private handleEnemyDestruction(payload: BridgeEnemyShipDestructionPayload): void {
        for (const [channelId, channel] of this.channels) {
            if (channel.targetActorId !== payload.actorId) {
                continue;
            }
    ...
    private drawProjection(
        ...
        const flickerIndex = Math.floor(animationMs / OUTGOING_SPAM_VFX.flickerFrameMs) % OUTGOING_SPAM_FLICKER.length;

        const flicker = OUTGOING_SPAM_FLICKER[flickerIndex];

        if (flicker === undefined) {
            throw new Error("Outgoing spam flicker table is empty");
        }

        const breath = Math.sin((animationMs / OUTGOING_SPAM_VFX.breathPeriodMs) * Math.PI * 2);

        const targetVisualBounds = this.getObjectVisualBounds(targetActorId);

        if (!targetVisualBounds) {
            return;
        }

        const targetCenter: ProjectionPoint = {
            x: targetVisualBounds.centerX,

            y: targetVisualBounds.centerY,
        };

        const fullDx = targetCenter.x - source.x;

        const fullDy = targetCenter.y - source.y;
        ...
        const directionX = fullDx / fullDistance;

        const directionY = fullDy / fullDistance;

        const end: ProjectionPoint = {
            x: source.x + directionX * OUTGOING_SPAM_VFX.lengthPx,

            y: source.y + directionY * OUTGOING_SPAM_VFX.lengthPx,
        };
        ...
        const outerEndHalfWidth = OUTGOING_SPAM_VFX.outerEndHalfWidth + breath * OUTGOING_SPAM_VFX.breathAmplitudePx;

        const coreEndHalfWidth =
            OUTGOING_SPAM_VFX.coreEndHalfWidth + breath * OUTGOING_SPAM_VFX.breathAmplitudePx * 0.45;
        ...
        this.drawConeBand({
            ...
            sourceHalfWidth: OUTGOING_SPAM_VFX.outerSourceHalfWidth,

            endHalfWidth: outerEndHalfWidth,
        ...
        this.drawConeBand({
            ...
            sourceHalfWidth: OUTGOING_SPAM_VFX.coreSourceHalfWidth,

            endHalfWidth: coreEndHalfWidth,
        ...
        this.graphics.fillStyle(secondaryTint, 0.28 * flicker).fillCircle(
            source.x,
            source.y,

            OUTGOING_SPAM_VFX.sourceGlowRadius,
        );

        this.graphics.fillStyle(tint, 0.9).fillCircle(
            source.x,
            source.y,

            OUTGOING_SPAM_VFX.sourceCoreRadius,
        );
    }
    ...
    private drawConeBand({
        ...
        const startCenter = this.lerpPoint(source, end, startT);

        const endCenter = this.lerpPoint(source, end, endT);

        const startWidth = Phaser.Math.Linear(sourceHalfWidth, endHalfWidth, startT);

        const endWidth = Phaser.Math.Linear(sourceHalfWidth, endHalfWidth, endT);

        this.graphics
            .fillStyle(tint, alpha)
            .beginPath()
            .moveTo(
                startCenter.x + normalX * startWidth,

                startCenter.y + normalY * startWidth,
            )
            .lineTo(
                endCenter.x + normalX * endWidth,

                endCenter.y + normalY * endWidth,
            )
            .lineTo(
                endCenter.x - normalX * endWidth,
