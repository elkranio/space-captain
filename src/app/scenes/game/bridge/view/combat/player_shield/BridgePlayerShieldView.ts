...
export default class BridgePlayerShieldView {
    ...
    constructor(
        ...
        this.shield = this.scene.add
            .image(
                BRIDGE_PLAYER_HULL_COMBAT_POINTS.shieldAnchor.x,

                BRIDGE_PLAYER_HULL_COMBAT_POINTS.shieldAnchor.y,
        ...
        this.scene.layers.get("vfx").add(this.shield);
    ...
    private handleDeployed(payload: BridgePlayerShieldSnapshotPayload): void {
    ...
    private handleEnded(payload: BridgePlayerShieldEndedPayload): void {
    ...
    private applySnapshot(payload: BridgePlayerShieldSnapshotPayload): void {
        const remainingMs = Math.max(0, payload.remainingDurationMs);

        this.shield.setVisible(true).setAlpha(getBridgeShieldAlpha(remainingMs));
    }
