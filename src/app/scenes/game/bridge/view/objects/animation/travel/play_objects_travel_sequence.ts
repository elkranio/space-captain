...
// Проигрывает локальный перелёт между двумя encounter objects.
//
...
export function playObjectsTravelSequence(
    ...
    playDeparturePhase(fromView, targetView, context);
}
...
function playDeparturePhase(
    ...
    const timer = context.scene.time.addEvent({
        ...
        callback: () => {
            ...
            playEmptySpacePhase(targetView, context);
        },
    });
...
function playEmptySpacePhase(targetView: BridgeObjectSpriteView, context: BridgeObjectsAnimationContext): void {
    const timer = context.scene.time.delayedCall(TRAVEL_EMPTY_SPACE_DELAY_MS, () => {
        context.clearActiveTimer();

        playArrivalPhase(targetView, context);
    });

    context.setActiveTimer(timer);
}

function playArrivalPhase(targetView: BridgeObjectSpriteView, context: BridgeObjectsAnimationContext): void {
