...
function replaceAll(
    ...
    // Our source style often breaks `OBJECT.property`
    // across lines:
    //
    //     OBJECT
    //         .property
    //
    // Semantic property migrations must therefore ignore whitespace
    // around the dot instead of assuming one-line formatting.
    const propertyMatch =
        /^([A-Z_]+)\.([A-Za-z0-9_]+)$/
            .exec(
                before,
    ...
    if (propertyMatch) {
        ...
        if (
            ...
            fail(
                'Expected at least ' +
                    expectedMinimum +
                    ' semantic property replacements in ' +
                    path +
                    ' for ' +
                    label +
                    ', found ' +
...
for (
    const patcher of [
        ...
        'apply_captain_dashboard_visual_dry_atom_v2.mjs',
    ]
) {
