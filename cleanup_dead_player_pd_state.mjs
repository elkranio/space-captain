    ...
    (text) => {
        text =
            replaceRegex(
                ...
                '',
                1,
                'remove dead player ShipPointDefenseState import from combat model',
            );
...
// The new-game snapshot contains literal installed-player-PD expectations.
// Remove those blocks after generic encounter-call cleanup.
editFile(
    ...
    (text) => {
        text =
            replaceRegex(
                ...
                '',
                2,
                'remove player PD from new-run expected snapshots',
            );
...
try {
    ...
    console.warn(
        'Patch succeeded, but could not delete patcher itself:',
        String(error),
    );
}
